...
interface ProcessingDebuggerProps {
  onStatusSyncComplete?: () => void;
}

export const ProcessingDebugger = forwardRef<{ onStatusSyncComplete: () => void }, ProcessingDebuggerProps>(
  ({ onStatusSyncComplete }, ref) => {
    ...
    const syncStatuses = async () => {
      try {
        ...
        console.log('Waiting 3 seconds for database changes to propagate...');
        await new Promise(resolve => setTimeout(resolve, 3000));
        
        // Refresh this debugger
        await checkDocumentStatus(true);
        
        // Notify parent component to refresh its data
        if (onStatusSyncComplete) {
          console.log('Notifying parent component to refresh...');
          onStatusSyncComplete();
        }
...
ProcessingDebugger.displayName = "ProcessingDebugger";
